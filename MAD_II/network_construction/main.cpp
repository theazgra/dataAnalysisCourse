...
int main(int argc, char **argv)
{
    ...
    args::Flag _constructFromVectorData(methodGroup, "construct-from-vector-data", "Construct network from vector data. <vector-file>", {"construct-from-vector-data"});
    ...
    if (_constructFromVectorData)
    {
        ...
        uint dim = irisDataset.size();
        NetworkMatrix irisMat = NetworkMatrix(dim, dim);

        for (uint row = 0; row < irisMat.rows(); row++)
        {
            for (uint col = row + 1; col < irisMat.cols(); col++)
            {
                float d = irisDataset[row].gauss_distance(irisDataset[col]);
                irisMat.at(row, col) = d;
                irisMat.at(col, row) = d;
            }
        }

        //irisMat.print_matrix();
        irisMat.filter_e_radius(0.75);
        irisMat.export_network("/home/mor0146/github/dataAnalysisCourse/MAD_II/eps.csv");
        //irisMat.print_matrix();
        //irisMat.filter_kNN(5);
        //irisMat.export_network("/home/mor0146/github/dataAnalysisCourse/MAD_II/knn.csv");
        printf("Constructed.\n");
        return 0;
    }
